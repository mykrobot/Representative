...
class SearchViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func findButtonTapped(sender: UIButton) {
        ...
        let searchTerm = pickerView.selectedRowInComponent(0)
        let searchString = states[searchTerm]
        
        RepresentativeController.getRepFromSearchTerm(searchString) { (reps) -> Void in
            if reps.count > 0 {
